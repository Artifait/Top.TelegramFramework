# Используем базовый образ с нужной версией .NET Runtime
...
# Копируем и собираем проект(Top.TelegramFramework.Examples.Simple)
COPY . ./
RUN dotnet restore Top.TelegramFramework.Examples.Simple/
RUN dotnet publish Top.TelegramFramework.Examples.Simple/ -c Release -o /app
...
ENTRYPOINT ["dotnet", "Top.TelegramFramework.Examples.Simple.dll"]